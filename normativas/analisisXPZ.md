...
Cuando la cadena de resolución llegue a un nodo hoja sin `ATTCUSTOMTYPE` ni `idBasedOn`, mapear el tipo con los datos presentes en el XPZ: la presencia de `Decimals` (incluso `0`) o de `Length`/`AttMaxLen` confirma `bas:Numeric` (tipo por defecto de GeneXus para dominios y atributos sin tipo declarado), produciendo `Integer (<longitud>)` o `Decimal (<longitud>, <decimales>)`. Si el nodo no expone ningún dato de tipo (sin `bas:`, sin `idBasedOn`, sin `Length` ni `Decimals`), agotar primero la evidencia de uso activo en el XML unificado: asignaciones, comparaciones, filtros, argumentos reales y parámetros formales exportados. Si después de ese recorrido el campo escalar continúa sin tipo, aplicar la convención operativa final `Integer` sin dimensión. Para la descripción, agotadas las fuentes explícitas del XPZ y del uso, usar el nombre de la variable/campo (`Descripcion = Campo`) como valor estable. Estas convenciones no se aplican a estructuras, colecciones ni SDT ausentes. No usar `ATT_PICTURE` como evidencia de tipo.

> **Nota sobre descripciones genéricas de dominio:** la descripción de un campo de entrada puede quedar genérica cuando la variable solo referencia un dominio compartido sin `Description` propia en la variable. Por ejemplo, una variable `FechaDesde` con `idBasedOn: Domain:Fecha` sin `Description` propia resuelve la descripción del dominio (`Fecha`), igual que otra variable `FechaHasta` del mismo dominio. Esa es la fuente explícita que manda la regla anterior: la descripción del dominio se conserva y el nombre de campo solo es el respaldo estable. Para obtener descripciones más específicas (p. ej. "Fecha Desde"), la vía correcta es completar la metadata del XPZ (propiedad `Description` de la variable o del atributo/dominio); el analizador no transforma nombres por heurística.
