...
- Archivo de configuración JSON en la raíz del proyecto: `apiglm-doc-config.json`, para datos ambiguos no confirmables desde el XPZ (base del package del endpoint publicado, rutas).
...
- Informe de revisión `documentacion/Generador/assets/apiglm-doc-review.json` con los ítems que requieren confirmación humana (obligatoriedad no resuelta, descripciones no derivables, endpoint sin base confirmada), cada uno con su evidencia requerida y un ejemplo.
...
**Configuración raíz `apiglm-doc-config.json`:**

```json
{
  "xpz": "xpz/LPS_COM.xpz",
  "inventario": "documentacion/Endpoints/assets/endpoints.json",
  "directorioSalida": "documentacion/servicios",
  "informeRevision": "documentacion/Generador/assets/apiglm-doc-review.json",
  "basePackage": {
    "APIGLM.Emision": "ar.com.glmsa.seguros.comercial.apiglm.emision"
  }
}
```

`basePackage` mapea un módulo GeneXus (prefijo del `fullyQualifiedName`) a la base del package confirmada por evidencia operativa. Si el módulo del wrapper no está en `basePackage`, el campo Endpoint se documenta como `PENDIENTE DE CONFIRMACIÓN`. `basePackage` se redefine manualmente en este archivo cuando se confirma un dato nuevo.
...
1. Crear `apiglm-doc-config.json` en la raíz con el esquema anterior y las carpetas `documentacion/Generador/binary/` y `documentacion/Generador/assets/`. Agregar a `.gitignore` las salidas del generador (`documentacion/Generador/assets/`).
...
8. Implementar `Resolver-Endpoint` (sección 7): nombre en minúsculas = `basePackage[módulo]` + ruta del módulo + `a` + procedimiento; si falta la base en la config → `PENDIENTE DE CONFIRMACIÓN`.
...
- [ ] `apiglm-doc-config.json` existe en la raíz con las claves `xpz`, `inventario`, `directorioSalida`, `informeRevision` y `basePackage`.
...
- **Sí:** Config JSON en la raíz con `basePackage`. El XPZ no confirma la base del package; el mapa permite registrarla por servicio y redefinirla después.
...
| Base del package no confirmada para un módulo | `PENDIENTE DE CONFIRMACIÓN` con la evidencia operativa; nunca construir la base por analogía. |
