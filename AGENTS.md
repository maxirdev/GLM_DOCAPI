...
- `configuracion.json` (raíz del repo) — configuración central multicliente (SPEC 19): `rutas.clientesRoot` (relativo a la raíz del repo o absoluto), `herramientas` globales (GeneXus, MSBuild, Pandoc, Typst), `exportacion.onlyModuleAPIGLM` y la colección `clientes`. Cada cliente define `id` (slug en minúsculas), `nombre`, `packagename`, `serviciosIgnorados` y `ambientes`. Cada ambiente define `id`, `nombre`, `tipo` (`test` o `prod`) y `kbPath` explícito; un cliente admite como máximo dos ambientes (un TEST y un PROD). El archivo no contiene `xpz` ni `kbPath` globales: el XPZ y la KB se derivan del contexto. El panel web activa un contexto sin inferir un XPZ automáticamente: el XPZ en verde surge solo de la exportación o de una selección explícita.
