...
Actualmente cubre el pipeline, configuración, selección no interactiva, inventario multi-XPZ, análisis de contratos, validación Markdown, visor, control de versiones esquema 2, staging/promoción y lock, historial de versiones, estado del control, el módulo común de utilidades (`binary/GLMUtilidades.ps1`) y la invocación real por proceso de `GenerarDocumento`, `GenerarPdfServicios`, `GenerarListaEndpoints` y `DiagnosticoIA`. Los temporales se escriben únicamente en `test/tmp/` y los logs de prueba en `test/Logs/`.
